...
echo '==> Setting MariaDB 12.1 repository'
...
{
    rpm --import --quiet https://archive.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-10
    ...
    dnf -q -y install https://rpms.remirepo.net/enterprise/remi-release-10.rpm
    dnf -q -y module enable php:remi-8.4
} &>/dev/null
...
cp /vagrant/config/21-php.conf /etc/httpd/conf.modules.d/21-php.conf
...
if [ $RUBY_VERSION ]; then
    ...
    dnf -q -y install gcc make openssl-devel readline-devel zlib-devel -y &>/dev/null
